...
from hparam_CLASS_pdc_2_la8_test import hparams as hp
...
def create_model(ema=False):
    ...
    # from models.three_d.vnet3d_dc_my import VNet
    # model = VNet(n_channels=1, n_classes=5, normalization='batchnorm',has_dropout=True,dc=True).to(device)
...
def test():
    ...
    val_util = MedData_val(val_image_dir, val_label_dir, )
    ...
    with torch.no_grad():
        ...
        for val_data in val_loader:
            ...
            # metric_channel1 = dice_metric_channel1.aggregate()
            # print('use time:{:.4f},mean dice:{:.4f},'.format(time2 - time1,torch.mean(metric_channel1)),metric_channel1,i)
            i=i+1
