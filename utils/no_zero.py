...
class Data_Preprocess:
    ...
    def do_select(self):
        ...
        print(self.bg/507," back")
        print(self.fg/507," fore")
        print(self.fb/507," fb")
        print(self.fc/507," fc")
        print(self.tb/507," tb")
        print(self.tec/507," tec")
        print(self.tic/507," tic")
        print(self.fb1/507," fb1")
        print(self.fc1/507," fc1")
        print(self.tb1/507," tb1")
        print(self.tec1/507," tec1")
        print(self.tic1/507," tic1")
    ...
    def select_dataset(self):
        self.img_ids = []
        for path in glob.glob(os.path.join(base_dir, f'image', '*.nii.gz')):
            img_id = path.split('/')[-1].split('image')[-1].split('.nii.gz')[0]
    ...
    def find_bound(self, data_list):
        for i, img_id in enumerate(data_list):
            label_path = os.path.join(base_dir, f'label', f'{img_id}.nii.gz')
    ...
    def do_select_dataset(self, data_list, tag):
        ...
        for i, img_id in enumerate(data_list):
            ...
            image_path = os.path.join(base_dir, f'image', f'{img_id}.nii.gz')
            label_path = os.path.join(base_dir, f'label', f'{img_id}.nii.gz')
            # shutil.copy(image_path,join(self.Base_path, f'image{tag}', f'{img_id}.nii.gz'))
            label = sitk.ReadImage(label_path)
            label_array = sitk.GetArrayFromImage(label)
            h,d,w = label_array.shape
            print(h,d,w)
            n = 384 * 384 * 160
            nozero = np.count_nonzero(label_array)
            fg = nozero / n

            fb = np.bincount(label_array[label_array==1])/n
            fc = np.bincount(label_array[label_array==2])/n
            tb = np.bincount(label_array[label_array==3])/n
            tec = np.bincount(label_array[label_array==4])/n
            tic = np.bincount(label_array[label_array==5])/n

            fb1 = np.bincount(label_array[label_array==1])/nozero
            fc1 = np.bincount(label_array[label_array==2])/nozero
            tb1 = np.bincount(label_array[label_array==3])/nozero
            tec1 = np.bincount(label_array[label_array==4])/nozero
            tic1 = np.bincount(label_array[label_array==5])/nozero

            self.fg = self.fg + fg
            self.bg = self.bg +(1-fg)

            self.fb = self.fb + fb
            self.fc = self.fc + fc
            self.tb = self.tb + tb
            self.tec = self.tec + tec
            self.tic = self.tic + tic

            self.fb1 = self.fb1 + fb1
            self.fc1 = self.fc1 + fc1
            self.tb1 = self.tb1 + tb1
            self.tec1 = self.tec1 + tec1
            self.tic1 = self.tic1 + tic1
