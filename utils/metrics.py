...
class ConfusionMatrix:
    ...
    def set_test(self, test):
        ...
        self.test = test.numpy()
        self.reset()

    def set_reference(self, reference):

        self.reference = reference.numpy()
        self.reset()
    ...
    def compute(self):
        ...
        self.test_empty = not np.any(a=self.test)
        self.test_full = np.all(a=self.test)
        self.reference_empty = not np.any(a=self.reference)
        self.reference_full = np.all(a=self.reference)
        # self.test_empty = not self.test.any()
        # self.test_full = self.test.all()
        # self.reference_empty = not self.reference.any()
        # self.reference_full = self.reference.all()
